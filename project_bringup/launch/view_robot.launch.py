...
def generate_launch_description():
    ...
    moveit_config = (
        ...
        .joint_limits(
            file_path=os.path.join(
                project_description_path, "config", "irb120_joint_limits.yaml"
            )
        )
        .robot_description_kinematics(
            file_path=os.path.join(
                project_description_path, "config", "irb120_kinematics.yaml"
            )
        )
    ...
    rviz = Node(
        ...
        arguments=[
            "-d",
            os.path.join(project_description_path, "rviz", "urdf_description.rviz"),
        ],
        output="screen",
