...
# This tutorial will launch the same as "getting started"
#   the difference was to add SVTL plugin to local_map
#   in the 'nav2_params.yaml'
def generate_launch_description():

    pkg_share = get_package_share_directory("project_description")
    nav2_bringup_path = get_package_share_directory("nav2_bringup")
    default_model_path = os.path.join(pkg_share, "urdf", "sam_bot", "sam_bot.sdf")
    default_rviz_config_path = os.path.join(pkg_share, "rviz", "nav2.rviz")
    bridge_config_path = os.path.join(
        pkg_share, "config", "sam_bot", "bridge_config.yaml"
    )
    ...
    robot_localization_node = Node(
        ...
        output="screen",
        parameters=[
            os.path.join(pkg_share, "config", "sam_bot", "ekf.yaml"),
            {"use_sim_time": LaunchConfiguration("use_sim_time")},
        ],
    )
